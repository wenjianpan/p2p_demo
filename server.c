...
// these value or functions are for stream speed control
struct timeval streamControlTimeTick;
struct timeval savedStreamControlTimeTick;
int savedSegID = 0;
int isTheFirstClient = 1;
int controlTimeTickIsUpdated()
{
	if(streamControlTimeTick.tv_sec > savedStreamControlTimeTick.tv_sec) return 1;
	else if(streamControlTimeTick.tv_sec == savedStreamControlTimeTick.tv_sec \
		&& streamControlTimeTick.tv_usec > savedStreamControlTimeTick.tv_usec) return 1;
	return 0;
}
//---------------------------------------------
...
struct _peer* getNewClient(int ourListenSocket) 
{
   ...
   SOCKLEN_T clientAddrLen = sizeof(clientAddr);
	 ...
	 if(Clients<MAXOUT)
       ...
       if(isTheFirstClient) { 
       	 getTimeOfDay(&streamControlTimeTick);
         isTheFirstClient = 0;
       }
	  ...
	  if(peerType == T_CLIENT) {
    	switch(nmsg[4]) {
         ...
         printf("from %d, recv MSG_KEEPALIVE\n", peer->ios.fd);
	  	...
	  	case MSG_IAMLEAVING:
#ifdef DEBUG
         printf("from %d, recv MSG_IAMLEAVING\n", peer->ios.fd);
#endif
...
#ifdef DEBUG
             printf("send segment: %d-%d\n",ourBufferMap.ourSessionID, segID);
#endif
             if(segID >= savedSegID ) {
#ifdef DEBUG
               printf("update stream control tick\n");
#endif
               getTimeOfDay(&streamControlTimeTick); // update the streamControlTimeTick
	  	  	   }
	  	  	}
	  	  }	
	  	  break;
...
int main(int argc, char *argv[])
{
	...
	if(argc!=2) {
		 printf("Usage: %s media-filename\n", argv[0]);
		 return -1;
	}
	if((fd = open(argv[1], O_RDONLY)) < 0){ // open media file
	...
	getTimeOfDay(&timeTick);
	savedStreamControlTimeTick.tv_sec = 0;
	savedStreamControlTimeTick.tv_usec = 0;
    ...
    list_for_each(curr, &ClientsList){
       ...
       handlePeerIncommingMsg(CNode, T_CLIENT);
        ...
        // if streamControlTimeTick is greater than savedStreamControlTimeTick
    if(controlTimeTickIsUpdated()) {    	    
      savedStreamControlTimeTick.tv_sec = streamControlTimeTick.tv_sec;
	    savedStreamControlTimeTick.tv_usec = streamControlTimeTick.tv_usec;
	    // read the media data from local file
	    // save the seg data to the right buffer, 8192 bytes;
	    // make sure ourSegID is <128 && >=0
	    if(ourSegID < 0) Error("ourSegID < 0, something error");
	    if(ourSegID > 127) // the last seg is sent out
	    {
	    	  changeBuffer();
	        ourBufferMap.ourSessionID++;
	        ourSegID = 0;
	    }
	    read(fd, &currBigBuf[ourSegID*8*1024], 8192);
	    setBit(&ourBufferMap, ourSegID);
#ifdef DEBUG
      printf("get segment: %d-%d\n",ourBufferMap.ourSessionID, ourSegID);
#endif
	    announceToClients(ourSegID);
	    savedSegID = ourSegID;
	    ourSegID++;
	    
	    // deal with if Buffer is full
